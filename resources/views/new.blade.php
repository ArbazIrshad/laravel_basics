...
<!-- TODO: CHANGE ID -->
<input type="hidden" name="restaurant_id" id="restaurant_id" value="rNQFVQIBOMV0AhJSodtB">
...
<script type="text/javascript">
	...
	var vendorId =  'rNQFVQIBOMV0AhJSodtB';
	var currentCurrency = '';
	...
	async function getVendorDetails(){
		...
		vendorDetailsRef.get().then( async function(vendorSnapshots){
			...
			console.log(specialOfferVendor);
		...
		console.log("specialOfferForHour"+specialOfferForHour);
...
console.log('HELP',vendorProduct);										totalProduct = totalProduct + 1;
														...
														productHtml += '<div class="media-body">';
															...
															if(vendorProduct.hasOwnProperty('disPrice') && vendorProduct.disPrice != '' && vendorProduct.disPrice != '0' ){
											                    if(currencyAtRight){
											                        ...
											                        productHtml=productHtml+'<p class="text-muted mb-0">'+vendorProduct.disPrice+''+currentCurrency+'  <span class="strick_price"><s>'+vendorProduct.price+''+currentCurrency+'</s>'+ '   '+vendorProduct.description  +'</span></p>';
											                    }else{
											                         productHtml=productHtml+'<p class="text-muted mb-0">'+''+currentCurrency+vendorProduct.disPrice+'  <span class="strick_price"><s>'+currentCurrency+''+vendorProduct.price+'</s>'+ '   '+vendorProduct.description + '</span></p>';
											                    }

											                }else{

											                	if(currencyAtRight){
											                        productHtml=productHtml+'<p class="text-muted mb-0">'+vendorProduct.price+''+currentCurrency+ '   '+ vendorProduct.description + '</p>';
											                    }else{
											                         productHtml=productHtml+'<p class="text-muted mb-0">'+currentCurrency+''+vendorProduct.price+'   ' + vendorProduct.description +'</p>';
											                    }
											                }
	...
	$( document ).ready(function() {
		...
		$(document).on("click", '.count-number-input-cart', function(event) {
			...
			$.ajax({
               ...
               data:{_token:'<?php echo csrf_token() ?>',restaurant_id:restaurant_id,id:id,quantity:quantity,specialOfferForHour:specialOfferForHour},
					...
					if(coupondata.resturant_id!=undefined && coupondata.resturant_id!=''){
						...
						if(coupondata.resturant_id==restaurant_id){
							...
							$.ajax({
				               ...
				               data:{_token:'<?php echo csrf_token() ?>',coupon_code:coupon_code,discount:discount,discountType:discountType,coupon_id:coupondata.id,specialOfferForHour:specialOfferForHour},
